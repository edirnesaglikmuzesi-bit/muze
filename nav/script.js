...
function openLang() {
    ...
    if (onIntro && introAudio) {
        // Intro ekranında: önce sesi çalmayı dene (tarayıcı unlock için)
        // Sonra modalı aç; kapanınca dil kontrolüne göre devam edilir
        introAudio._langWasPlaying = !introAudio.paused;
...
function closeLang() {
    ...
    if (onIntro) {
        // Türkçe seçiliyse sesi başlat, başka dil seçildiyse durdur
        if (introAudio) {
            if (currentLang === 'tr') {
                introAudio.play().catch(() => {});
            } else {
...
function setLang(code){
  ...
  // Modalları hemen kapat — ses yönetimi closeLang içinde yapılır
closeLang();
closeMenu();
...
window.addEventListener('DOMContentLoaded',()=>{
  ...
  introAudio.play().then(()=>{window._introStarted=true;window._introUnlocked=true;}).catch(()=>{
    ...
    function _onFirstTouch(){
      ...
      _iEvents.forEach(ev=>document.removeEventListener(ev,_onFirstTouch));
